...
function Resolve-TargetFrameworkRoot {
    if ($TargetFrameworkRootPath) {
        $frameworkList = Join-Path $TargetFrameworkRootPath '.NETFramework\v4.8\RedistList\FrameworkList.xml'
        if (-not (Test-Path -LiteralPath $frameworkList)) {
            throw "The supplied targeting pack root does not contain .NET Framework 4.8: $TargetFrameworkRootPath"
        }
        return (Resolve-Path -LiteralPath $TargetFrameworkRootPath).Path
    }
