...
{
    public partial class PrivateUpdatePopup : UserControl
    {
        ...
        public PrivateUpdatePopup(string currentVersion, string latestVersion, string latestNotes, Action onUpdate,
            Func<Task<string>> loadFullChangelog)
        {
            ...
            _latestNotes = string.IsNullOrWhiteSpace(latestNotes)
                ? GetResourceText("private_up_release_notes_unavailable")
                : latestNotes;
        ...
        private async void OnShowMoreClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                SetNotesText(_latestNotes);
                ShowMoreButton.Content = GetResourceText("private_up_show_more");
            ...
            if (string.IsNullOrWhiteSpace(_fullChangelog))
            {
                ...
                ShowMoreButton.Content = GetResourceText("private_up_loading_changelog");
            ...
            if (string.IsNullOrWhiteSpace(_fullChangelog))
            {
                ...
                SetNotesText(string.Concat(
                    ...
                    Environment.NewLine,
                    Environment.NewLine,
                    GetResourceText("private_up_changelog_failed")));
                return;
            }

            SetNotesText(_fullChangelog);
            ShowMoreButton.Content = GetResourceText("private_up_show_less");
