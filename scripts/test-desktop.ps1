...
# The fork updater must remain available independently of upstream notifications.
$privatePopup = $assembly.GetType('WandEnhancer.View.Popups.PrivateUpdatePopup', $true)
$privateConstructor = $privatePopup.GetConstructor(@([string], [string], [string], [Action], [Func[System.Threading.Tasks.Task[string]]]))
Assert-Equal ($null -ne $privateConstructor) $true 'Private update popup action contract'
$forkVm = $assembly.GetType('WandEnhancer.View.MainWindow.MainWindowVm', $true)
Assert-Equal ($null -ne $forkVm.GetProperty('IsPrivateUpdateAvailable')) $true 'Private update badge state'
Assert-Equal ($null -ne $forkVm.GetProperty('UpdateCommand')) $true 'Private update command'
